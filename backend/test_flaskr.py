...
class TriviaTestCase(unittest.TestCase):
    ...
    def test_delete_question(self):
        ...
        delete_json = {"currentCategory": 3}
        # Click on the trash icon
        self.client().delete(
            f"/questionsDelete/{post_id}",
            data=json.dumps(delete_json),
    ...
    def test_delete_wrong_method_error_handling(self):
        ...
        response = self.client().get(
            f"/questionsPost",
            data=json.dumps(self.test_post),
            content_type="application/json",
        )
        json_response_data = json.loads(response.data)
    ...
    def test_search(self):
        insert_test_post()
        search_term = {"searchTerm": "Test Question?"}
        # First do a search by SQL directly from the database
        questions_search_query = Question.query.filter_by(
            question="Test Question?"
        ).all()
        response = self.client().post(
            f"/questionsSearch",
        ...
        self.assertEqual(
            len(json_response_data["questions"]), len(questions_search_query)
        )
        delete_test_post()
    ...
    def test_search_404(self):
        insert_test_post()
        search_term = {"searchTerm": "Not existing term"}
    ...
    def test_quizzes(self):
        ...
        quiz_post_data = {"previous_questions": [], "quiz_category": quizCategory}
        response = self.client().post(
            "/quizzes", data=json.dumps(quiz_post_data), content_type="application/json"
        )
        json_response_data = json.loads(response.data)
        self.assertEqual(response.status_code, 200)
